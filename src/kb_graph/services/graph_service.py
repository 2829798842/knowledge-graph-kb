...
from kb_graph.data.models import (
    Chunk,
    Document,
    EdgeType,
    GraphEdge,
...
from kb_graph.contracts.api.graph_contracts import GraphEdgeRead, GraphNodeRead, GraphResponse
from kb_graph.contracts.extraction_contracts import ExtractionResult
from kb_graph.services.vector_store_service import LanceDbVectorStore, VectorRecord
...
def build_graph_response(
    ...
    *,
    ...
    if document_id:
        ...
        document_chunks: list[Chunk] = session.exec(select(Chunk).where(Chunk.document_id == document_id)).all()
        base_node_ids.update(chunk.node_id for chunk in document_chunks)

        related_edges: list[GraphEdge] = session.exec(
            select(GraphEdge).where(
                or_(
                    GraphEdge.source_node_id.in_(base_node_ids),
                    GraphEdge.target_node_id.in_(base_node_ids),
                )
            )
        ).all()
        ...
        nodes: list[GraphNode] = session.exec(select(GraphNode).where(GraphNode.id.in_(node_ids))).all()
        edges: list[GraphEdge] = [
            edge
            for edge in session.exec(select(GraphEdge).where(GraphEdge.source_node_id.in_(node_ids))).all()
...
def fetch_nodes_by_ids(session: Session, node_ids: Iterable[str]) -> list[GraphNode]:
    ...
    if not node_list:
        return []
    return session.exec(select(GraphNode).where(GraphNode.id.in_(node_list))).all()
