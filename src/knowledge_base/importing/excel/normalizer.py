...
def _prepare_sheet(
    *,
    ...
    configured_header_row = int(rule.get("header_row")) if rule and rule.get("header_row") else None
...
def _infer_primary_key(header_keys: list[str]) -> str:
    for candidate in header_keys:
        ...
        if normalized_candidate in {"id", "编号", "编码"} or normalized_candidate.endswith("id"):
            return candidate
